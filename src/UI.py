...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.top_menu = tk.Frame(self, width=MAX_WIDTH, height=MAX_HEIGHT / 4, bg=grey)

        self.top_menu.grid(row=0, column=0, pady=4)

        # drawArea is the Canvas where rectangles'll be drawn
        self.drawArea = tk.Canvas(
            self, width=MAX_WIDTH, height=MAX_HEIGHT, bg=lightBlue
        )
        ...
        self.speedScale = TickScale(
            self.top_menu, from_=0.005, to=2, digits=3, orient="horizontal"
        )
        self.speedScale.grid(row=0, column=3, columnspan=3, padx=15, pady=0, sticky="W")

        # second ROW
        tk.Label(self.top_menu, text="Size: ", bg=grey).grid(
            row=1, column=0, padx=15, pady=3, sticky="SW"
        )
        self.sizeEntry = TickScale(
            self.top_menu, from_=1, to=200, digits=0, orient="horizontal"
        )
        self.sizeEntry.set(200)
        ...
        self.maxEntry = TickScale(
            self.top_menu, from_=1, to=200, digits=0, orient="horizontal"
        )
        self.maxEntry.set(200)
    ...
    def Draw(self, color=list, generate_flag=False):
        ...
        rect_width = (MAX_WIDTH - MAX_WIDTH / (len(self.data))) / (len(self.data))
